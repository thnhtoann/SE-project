...
- `.claude/skills/` — auto-invoked workflows: backend (`django-app-scaffolder`, `drf-endpoint`, `django-test-writer`) and frontend (`next-page-scaffolder`, `redux-slice-generator`, `i18n-key-sync`), `archify` (generates architecture/workflow/sequence/data-flow/lifecycle diagrams as self-contained HTML — [tt-a1i/archify](https://github.com/tt-a1i/archify), installed via `npx skills`), plus the `example-skill` template and the `speckit-*` skills (see Spec Kit below)
