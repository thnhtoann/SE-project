...
const Header = () => {
    ...
    // "Messages" box shows just the order-arrival notifications (Lazada
    // orders confirmed via push -- see omnichannel/lazada.py's
    // _notify_lazada_order) out of the same feed the bell shows in full.
    const orderMessages = notifications.filter((n) => n.notification_type === 'lazada_order_received');
                                ...
                                <ul className="w-[300px] !py-0 text-xs text-dark dark:text-white-dark sm:w-[375px]">
                                    <li className="mb-5" onClick={(e) => e.stopPropagation()}>
                                        <div className="relative !h-[68px] w-full overflow-hidden rounded-t-md p-5 text-white hover:!bg-transparent">
                                            ...
                                            <h4 className="relative z-10 text-lg font-semibold">{t('new_orders')}</h4>
                                        </div>
                                    </li>
                                            ...
                                            {orderMessages.map((message) => {
                                                return (
                                                    <div key={message.id} className="flex items-center px-5 py-3">
                                                        <div className="grid h-9 w-9 shrink-0 place-content-center rounded-full bg-success-light text-success dark:bg-success dark:text-success-light">
                                                        ...
                                                        <button type="button" className="text-neutral-300 hover:text-danger" onClick={() => removeNotification(message.id)}>
                                                            <IconXCircle />
                                                        </button>
                                                    </div>
                                                );
